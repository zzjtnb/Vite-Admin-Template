    ...
    <nav class="mt-10">
      <ul>
        <li v-for="(item,index) in frameInRoutes">
          <router-link class="flex items-center mt-4 py-2 px-6 border-l-4" :class="[$route.path === item.path ? activeClass : inactiveClass]" :to="item.path" v-if="!item.children">
            <i :class="item.icon" class="fas pr-0 md:pr-3"></i>
            <span class="mx-4">{{item.name}}</span>
          </router-link>

          <div v-else class="text-gray-500 items-center mt-4 py-2 px-6" @click.native="dropdown = !dropdown">
            <i :class="item.icon" class="fas pr-0 md:pr-3"></i>
            <span class="mx-4">{{item.name}}</span>
            <i class="fa fa-angle-down pl-5"></i>
            <ul v-show="dropdown">
              <li v-for="item in item.children">
                <router-link :class="[$route.path === item.path ? activeClass : inactiveClass]" :to="item.path">
